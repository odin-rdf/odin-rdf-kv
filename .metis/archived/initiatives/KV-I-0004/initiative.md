...
- **REQ-009:** `env_resident_check` (D10) counts the map's pages actually present in the process, by asking the OS. *(Amended 2026-09-24, KV-T-0019: **on Linux.** macOS has no per-range source for this, so there `env_resident_check` returns `.Unsupported`, and the macOS success-criterion test checks the process's `task_info` `resident_size` against a baseline taken after `env_open` instead; see D10.)* *(Amended 2026-09-24, KV-T-0024: the macOS test checks the file-backed part of the resident size, `task_info` `external`; see D10.)*
...
- **D8: watermarks.** With budget B chunks: `env_sweep` does nothing at or below **B**, and above it evicts down to **7/8 B**; a thread whose page_ptr takes the count past **B + 2 chunks** (the hard watermark) evicts until the count is back to B itself, then continues. The criterion's "within 1–2 chunks" is the hard watermark's margin. An application that never sweeps still stays within B + 2 chunks, only with the eviction cost landing on reads. *(Amended 2026-09-24, the owner's decision during KV-T-0023: **the end of every transaction evicts, and inline eviction goes down to 7/8 B.** The application does not call `env_sweep` after requests: it calls `env_sweep(env, 0)` only after hours without a request, to put the store to sleep. So while the store is in use the budget is kept by the store itself: at the end of every transaction (in `txn_abort`, which `txn_commit` ends through too), once the transaction has released the reader table and the writer mutex, if the estimate is above B it evicts down to **7/8 B** through the same CLOCK code with `try_lock` (under budget: one atomic load). Inline eviction at **B + 2** stays as the backstop for one transaction that reads far past the budget, and now evicts down to **7/8 B** too, not back to B. 7/8 is a named constant (`LOW_WATERMARK_EIGHTHS`), which may later become tunable or self-tuning. `Stats` counts the three paths separately (`sweeps`, `txn_end_evictions`, `inline_evictions`).)* *(Amended 2026-09-24, KV-T-0023, **open**: with `try_lock` at the hard watermark, B + 2 holds for one thread but not under concurrency. A thread that passes B + 2 while another thread is evicting doesn't wait, and the other threads fault chunks in faster than one evictor removes them, so the estimate reached 36–96 chunks with B = 24 (4 readers and a writer, a working set within the budget). Waiting for the lock at the hard watermark only keeps it within B + 2 + one chunk per thread (28–29 seen). The choice is the owner's; see KV-T-0023.)* *(Decided 2026-09-24, option (a) of KV-T-0023: **at the hard watermark a thread waits for `evict_mutex`** (`mutex_lock`), then evicts whatever is still above 7/8 B; the end of a transaction and `env_sweep` keep `try_lock`. The wait falls only on threads already past B + 2, as back-pressure. Measured bound under concurrency: **B + 2 plus about one chunk per thread** — with B = 24, 4 readers and a writer, the highest estimate seen after a transaction was 28–29 chunks in every run on macOS (debug, `-o:speed`, TSan) and Linux arm64 and amd64 (debug, `-o:speed`). Invariant that comes with it: no code holding `evict_mutex` may reach `page_ptr`, or it waits on itself; eviction reads no page.)*
- **D9: sweeping is driven by the application; the store starts no thread (Q3).** `env_sweep(env, target := -1) -> (evicted: int)` runs CLOCK over the chunk table under `evict_mutex`: a chunk with `referenced` set has it cleared and survives; one without is evicted. With the default target it evicts only when above B, down to 7/8 B, so calling it after every request costs one atomic load when there's nothing to do. An explicit `target` (in bytes, 0 allowed) trims below the budget, for an application that wants an idle store to give its mapped pages back. **Target 0 is the sleep path** for the first deployment, whose main RDF store unloads itself on inactivity: it drops every mapped page while the env stays open, and unlike `env_close` it leaves slices valid and needs no reopen. What remains is the `Env`, the chunk table, the reader table and `Env.free`. `env_close` plus `env_open` on wake is the alternative when even that should go (the testing strategy measures both). It's safe from any thread, with or without a transaction open, and uses `try_lock`: a second concurrent call returns 0 at once rather than waiting. Inline eviction at the hard watermark goes through the same code. **Why no thread is enough:** the estimate only grows through `page_ptr`, which only runs inside a transaction, which in this deployment only runs inside a request. So a store with no requests has a resident count that isn't moving, and sweeping after requests covers every point where it can grow. Eviction clears `resident` first (and decrements the count), then evicts the range, so a racing reader can only cause an overestimate, never an underestimate (the vision's order). Evicting: **Linux** `madvise(MADV_DONTNEED)`, which drops the mapping's page table entries and leaves the page cache; **macOS** `mmap(MAP_FIXED)` of the same file range over itself, then `MADV_RANDOM` again, because `MADV_DONTNEED` is only a hint there. Both keep every address valid. Chunks past the end of the file are never marked resident and are skipped. *(Amended 2026-09-24, KV-T-0019: **measured, and the methods hold.** macOS: the remap is the only method that lowers the resident size and keeps the page cache, and 4 readers checking every word saw no fault and no wrong byte across ~800,000 remaps in 10 s, TSan clean; `MADV_DONTNEED` has no measurable effect there. Linux: `MADV_DONTNEED` drops the present bits and RSS and keeps the page cache. Three things added: **call `madvise`, not `posix_madvise`** (glibc makes `POSIX_MADV_DONTNEED` a no-op; measured); `msync(MS_INVALIDATE)` evicts on macOS but drops the page cache for every process, so a refault goes to disk (~450 µs per 256 KiB against ~5 µs), and is not used; and Linux maps 64 KiB per fault (fault-around) despite `MADV_RANDOM`, aligned, so it stays inside one D7 chunk. Evicting a 256 KiB chunk costs 2–7 µs and refaulting it from the page cache about 5 µs.)* *(Amended 2026-09-24, KV-T-0023, correcting the usage above: **`env_sweep` is not called after every request.** The application calls `env_sweep(env, 0)` when the process has seen no request for many hours, to put the store to sleep; "why no thread is enough" now rests on the end of every transaction evicting (D8, amended), not on sweeps after requests. The default target stays available, but no application needs it. Target 0 evicts every chunk below the highest one ever made resident with one call, counted or not, so the sleep path also drops pages faulted in through slices held across an earlier eviction (Q4), which CLOCK never sees. And the map is now reserved at a chunk-aligned address: Linux's fault-around window is aligned by address, not by file offset, so with `mmap`'s page alignment alone it could cross into a neighbouring chunk, which the estimate then missed (seen in KV-T-0023's test, with the map 60 KiB off a 64 KiB boundary).)*
- **D10: `env_resident_check`, on demand.** Counts the map's pages that are present, in chunk-sized units, over the file's extent (not all of `map_size`). **Linux:** the present bits of `/proc/self/pagemap` (never `mincore`, which reports the page cache). **macOS:** `mincore`, if the measurement (task 1) confirms it errs only toward overcounting; otherwise the per-region resident count from `mach_vm_region`. It's a statistic and a test oracle: it doesn't correct the estimate (see Q4). *(Amended 2026-09-24, KV-T-0019: **Linux holds**: `pagemap` present bits and `smaps` agree exactly with the RSS change, and `mincore` reports the page cache (64 MiB for a mapping nothing touched). **The macOS half does not hold, and its replacement is open.** Both `mincore` and `mach_vm_region` count the file's pages in the page cache, not the ones mapped into the process: 64 MiB before any touch and unchanged by a remap that halved the RSS. Neither can check the estimate, and no per-range source that follows the mapping was found; only the process-wide `task_info` `resident_size` does. The options and a recommendation are in KV-T-0019's status update, awaiting the owner's decision.)* *(Decided 2026-09-24, the owner choosing option 1 of KV-T-0019: **on macOS `env_resident_check` returns a new `Error` value, `.Unsupported`**, rather than reusing `Invalid_Argument`, since the call is valid and the platform simply can't answer it; the resident figure it returns is 0. **Linux uses `pagemap`** as planned. **The macOS success-criterion test** reads the process's `task_info` `resident_size` (and `phys_footprint` for the pool) against a baseline taken right after `env_open`, in a test process that holds nothing else of size, so the figure there is mapped pages plus the dirty pool plus noise. KV-T-0024 adds `.Unsupported` and implements both.)* *(Amended 2026-09-24, KV-T-0024: **the macOS figure is the file-backed resident size, `task_info`'s `external`,** above the baseline, plus `Stats.dirty_committed` for the pool, rather than the whole `resident_size`. `resident_size` also carries what macOS malloc keeps resident after a free (4–8 MiB in the criterion test, staying after `env_close`) and, under `-sanitize:thread`, the sanitizer's shadow of every byte read (three times the file). `external` follows the map exactly (+20,692,992 bytes after a full scan of a 20.7 MB tree, the same as `resident_size`). `resident_size` is still logged, and stayed within the criterion's limit too. On Linux a `pagemap` walk isn't atomic, so the criterion counts only samples during whose walk no chunk was faulted in or evicted.)*
...
- **Q3 (settled 2026-09-24): no sweeper thread.** The application calls `env_sweep`, for example after each HTTP request, and inline eviction at the hard watermark is the backstop (D8, D9). This saves 250 threads per server in the first deployment. It departs from the vision, which describes a background thread, and the vision is amended when this initiative completes. *(Amended 2026-09-24: the application doesn't call `env_sweep` after each request; the end of every transaction keeps the budget, and `env_sweep(env, 0)` is the sleep path after hours of inactivity. See D8 and D9.)*
...
All six tasks (KV-T-0019 to KV-T-0024) are complete. The ordinary suite has 161 tests and passes on macOS arm64 (debug, `-o:speed`, ASan; the threaded tests twice under TSan) and on Linux arm64 and amd64 (debug, `-o:speed`). The success-criterion test (`-define:KV_CRITERION=true`) passes on macOS arm64 and Linux arm64 and amd64, debug and `-o:speed`. **The steady-state tests ran once each, debug only, with 10³ commits** (plateau, full map, long reader, churn; also with the smallest pool in KV-T-0021). **The full-length `scripts/test.sh --steady` was not run in this initiative, at the owner's request.** Each task's status section records its decisions, measurements and deliberate-bug checks. The initiative itself awaits the owner's review.

### Exit criteria

| Requirement | Status | Evidence |
|---|---|---|
| REQ-001 options, `Invalid_Argument` | ✓ | `test_dirty_budget_options`, `test_chunk_options` (defaults, rounding, minimums, negative values) |
| REQ-002 dirty memory never above `dirty_budget` | ✓ | `expect_pool_within` after every operation in the spill tests and `run_model`; `test_spill_ten_times_the_pool` (≥ 10 × the 49-page pool in one transaction); `test_model_randomized_min_pool` (~11,000 spills a run); the criterion test (the writer's 1,500-message transactions; peak `dirty_committed` 3.97–3.98 MiB of 4) |
| REQ-003 spilling invisible | ✓ | every key through `get` and both cursor directions before and after each commit in `test_spill_ten_times_the_pool`; the model and the isolation test with the smallest pool |
| REQ-004 abort after spilling | ✓ | `test_spill_abort` (snapshot, `ready`, `pending` identical; `space_check` after the next commit) |
| REQ-005 overflow and free-list runs written directly | ✓ | `test_spill_value_larger_than_pool` (at most 2 dirty pages for a value 10 × the pool); `test_freelist_longer_than_pool` (a 59-page run with a 49-page pool) |
| REQ-006 estimate, eviction to the budget with or without `env_sweep` | ✓ | `evict_test.odin`; `test_evict_budget_no_sweep` (B + 2 plus about one chunk per thread under TSan, both platforms); the criterion test (no `env_sweep` call; 1,448–19,340 transaction-end and 12,877–27,463 inline evictions per 10 s run) |
| REQ-007 slices valid across eviction | ✓ | `test_evict_held_slices`, `test_sweep_concurrent` (TSan), `test_evict_across_writes`; the criterion test checks every value it reads against its key |
| REQ-008 `env_stats` figures, from any thread | ✓ | `test_env_stats`, `test_dirty_pages_live`; the criterion's sampler reads them during the load (TSan clean) |
| REQ-009 `env_resident_check` | ✓ (Linux); `.Unsupported` on macOS by decision | `test_resident_check`: within the estimate after reads and after transaction-end evictions, 0 after `env_sweep(env, 0)`; on macOS `.Unsupported`, and the same checks against `task_info` |
| NFR-001 reads allocate nothing, no lock on the fast path | ✓ | `test_reads_allocate_nothing`; `chunk_touch` compiles to a shift, loads, a compare and a branch (KV-T-0022) |
| NFR-002 read throughput within 5% | ✓ gets, **missed for scans (+9%), accepted** | gets within noise, 4 threads included; a tight cursor scan +1.4–1.5 ns per entry (KV-T-0022; the owner accepted it) |
| NFR-003 no format change | ✓ | no change to the page, meta or free-list layout |
| NFR-004 steady state with a small pool | ✓, reduced | the four steady tests with the 49-page pool at 10³ commits (KV-T-0021) and with the default pool (KV-T-0024); full length skipped at the owner's request |
| NFR-005 fixed structures under 1 MB | ✓ | 4 KiB chunk table for a 1 GiB map at 256 KiB chunks; 40–68 KiB in all after the criterion runs (below) |
| **The success criterion** | ✓ | below |

The deliberate-bug checks of the testing strategy each failed a test: spilling inside an operation, copy-on-write of a spilled page, skipping `file_grow` before a spill, freeing a spilled page to `freed` (KV-T-0021); skipping an overflow run's chunks, and `freelist_load`'s (KV-T-0022); evicting before clearing `resident`, skipping the `referenced` check, losing `MADV_RANDOM` after a macOS remap, checking at commit only (KV-T-0023); `env_resident_check` counting the page cache or reading the wrong range, macOS returning `.None`, and a store with no eviction under the criterion test (KV-T-0024).

### The success criterion (measured)

A 100 MB database of conversations (1,400 × 40 messages, one entry per message, one in ten an overflow value of 2–16 KiB). Two readers, a scanner and a writer ran for 10 s with **no `env_sweep`**, while a sampler read the resident figure plus the dirty pool, and the test asserted it stays within `mapped_budget + dirty_budget + 2 chunks`. **Linux:** `env_resident_check` plus `dirty_committed`, only samples during whose `pagemap` walk nothing was faulted in or evicted. **macOS:** `task_info` `external` above a baseline taken after `env_open`, plus `dirty_committed`. 0 samples over the limit in every run:

| platform | 256 KiB / 15 MiB + 4 MiB (limit 19.50 MiB) | 64 KiB / 8 MiB + 4 MiB (limit 12.12 MiB) |
|---|---|---|
| macOS arm64 (`-o:speed`, debug) | peak 8.59, 7.16 MiB | peak 7.50, 7.34 MiB |
| Linux arm64 (`-o:speed`, debug) | peak 9.85, 9.54 MiB | peak 12.04, 12.03 MiB |
| Linux amd64 (`-o:speed`, debug) | peak 10.04, 9.92 MiB | peak 11.97, 12.02 MiB |

The estimate peaked at B + 2 to B + 4 chunks. On macOS the whole process's `resident_size` stayed within the limit as well (11.42 and 7.80 MiB). **Drift (Q4):** in no consistent sample was the OS above the estimate, and at the end of each Linux run no chunk was present and uncounted. The OS figure averaged 14–37% of the estimate with pages read here and there in 256 KiB chunks, and 82–101% with 64 KiB chunks on Linux, where fault-around maps a whole chunk. No correction is needed.

### Capacity-planning figures (per store, `-o:speed`)

Method: the criterion test (`tests/criterion_test.odin`). "Mapped" is `env_resident_check` on Linux and `task_info` `external` above a baseline on macOS; "dirty" is `Stats.dirty_committed`; fixed structures are the store's own sizes. Linux ran in OrbStack's VM (kernel 7.0.14, arm64; amd64 emulated), not on bare metal; cgroup accounting was not measured. Design bound = `mapped_budget + 2 × chunk_size + dirty_budget + fixed`.

| figure | Linux arm64, 256 KiB / 15 MiB | Linux arm64, 64 KiB / 8 MiB | macOS arm64, 256 KiB / 15 MiB | macOS arm64, 64 KiB / 8 MiB |
|---|---|---|---|---|
| design bound | 19.54 MiB | 12.19 MiB | 19.54 MiB | 12.18 MiB |
| active, worst case: peak mapped + peak dirty (10 s mixed load) | 6.00 + 3.98 = 9.98 MiB | 8.12 + 3.97 = 12.09 MiB | 5.27 + 3.98 = 9.25 MiB | 3.72 + 3.98 = 7.70 MiB |
| active, worst case, the store's own bound (peak estimate × chunk + 4 MiB) | 19.5 MiB | 12.1 MiB | 20.0 MiB | 12.25 MiB |
| active, typical: mapped, median / max (single-entry requests) | 4.25 / 4.88 MiB | 7.50 / 8.00 MiB | 1.08 / 1.27 MiB | 1.98 / 2.20 MiB |
| active, typical: dirty | 0 between requests; 20 KiB during a write | 0 / 20 KiB | 0 / 16–32 KiB | 0 / 16 KiB |
| asleep (`env_sweep(env, 0)`): mapped / dirty | 0 / 0 | 0 / 0 | 0 / 0 | 0 / 0 |
| asleep: fixed structures | 42 KiB | 67 KiB | 41 KiB | 55 KiB |
| — `Env` / chunk table / reader table | 680 B / 4 KiB / 256 B | 680 B / 16 KiB / 256 B | 680 B / 4 KiB / 256 B | 680 B / 16 KiB / 256 B |
| — free list (16 B per record) | 8 KiB (1,019 records) | 20 KiB (2,515) | 7 KiB (890) | 9 KiB (1,101) |
| — pool bookkeeping (1,024 slots) | 28 KiB | 28 KiB | 28 KiB | 28 KiB |
| after `env_close` | 0 | 0 | 0 | 0 |
| reserved address space (virtual, not RAM) | 1 GiB map + 4 MiB pool | same | same | same |

Linux amd64 matched arm64 within a few percent (worst 6.06 + 3.98 and 8.12 + 3.97 MiB; typical 4.25 / 4.88 and 7.36 / 7.92 MiB). A store in use sits at its budget in the estimate; what the OS holds is lower when reads touch only part of each chunk. The free list is the one fixed structure that grows: 16 B per record (20,000–47,000 records after deleting half the conversations with a reader held: 0.3–0.75 MiB).

### Measurements

| | macOS arm64 | Linux arm64 | Linux amd64 (emulated) |
|---|---|---|---|
| end-of-transaction check, under the budget | 1.2 ns | 1.2–1.3 ns | 1.9–2.0 ns |
| `txn_abort` when it evicts (256 KiB: ~9.5 chunks; 64 KiB: ~17.5) | 12.7–13.6 µs; 25.9 µs | 10.8 µs; 18.8 µs | 17.4–17.9 µs; 31.9 µs |
| sleep, `env_sweep(env, 0)` from the budget (60 × 256 KiB; 128 × 64 KiB) | 64–76 µs; 87 µs | 47 µs; 89 µs | 79–88 µs; 155 µs |
| wake, `env_open`, free list as built (8 records) | 25–27 µs | 7.4 µs | 11.6 µs |
| wake, `env_open`, long free list | 2.1 ms (20,538 records) | 5.2 ms (47,164) | 7.0 ms (45,649) |
| spill, per page (KV-T-0021) | 0.7–1.9 µs | 1.5–1.7 µs | |
| evict a 256 KiB chunk / refault it from the page cache (KV-T-0019) | 1.8 / 4.9 µs | 3.6 / 4.7 µs | 6.7 / 5.8 µs |

Sleep against close: asleep, the store keeps its heap (the table above) and its address-space reservations, and waking costs nothing up front; after close it keeps nothing, and waking costs `env_open` (above: about 0.1 µs per free-list record). Either way the next reads fault their chunks back in from the page cache.

### Deviations from the design
- **No sweeper thread (Q3), and no `env_sweep` after requests (D8, D9 as amended):** the end of every transaction evicts above B down to 7/8 B (`try_lock`); a read past B + 2 **waits** for `evict_mutex` and evicts to 7/8 B; `env_sweep(env, 0)` is the sleep path after hours without a request, and evicts the whole range below `end` with one call, counted or not.
- **The map is reserved at a chunk-aligned address** (Linux fault-around is aligned by address).
- **macOS:** the pool is released with an anonymous `MAP_FIXED` remap (D1); `env_resident_check` returns `.Unsupported` (D10); the criterion measures `task_info` `external`, not the whole `resident_size` (D10, amended in KV-T-0024).
- **Spilling:** only between operations, enforced by an assertion (`Write_State.in_op`), since LRU hides the violation; adjacent pages written in one call; `Stats.spills` counts pool pages only; the commit writes the free-list run before the remaining dirty pages; a failed spill doesn't poison the transaction.
- **Accounting:** the meta pages and `space_check`'s reads are accounted; `free_list_bytes` is the arrays' capacity; commit and release in the pool work on granules (the OS page, 16 KiB on Apple Silicon).
- **NFR-002 is missed for tight cursor scans (+9%)**, accepted by the owner.
- **The criterion test** adds consistent-sample filtering on Linux and a 200 µs pause between reader and scanner requests (without it almost no `pagemap` walk ran undisturbed); its typical load runs without pauses, the store having no time-driven behaviour.

### Known limitations
- **The mapped budget is soft:** B + 2 chunks for one thread, and about one more chunk per thread under concurrency (D8). With 64 KiB chunks on Linux, where every touched chunk is fully resident, the criterion's margin was about one chunk.
- **Undercounts (Q4), measured and not corrected:** pages read through slices held across an eviction; and pages an operation reads after an inline eviction inside that same operation evicted the chunk it had just marked (CLOCK's second turn when every chunk is referenced; 2–3 chunks seen with a budget of 8). Both are dropped by `env_sweep(env, 0)`, and neither showed at the end of the criterion runs.
- **`env_resident_check` is Linux only,** and it isn't atomic: a `pagemap` walk over 100 MB takes 0.1–0.3 ms, and chunks faulted in and evicted meanwhile can all be counted.
- **A value larger than a few chunks is resident in full while it is read**; store conversation messages as separate entries.
- **Outside the budget:** `Env.free` (16 B per record, and growing while long readers hold pages), the reader table, the chunk table, the pool's bookkeeping and a write transaction's page lists.
- **Step 7** must add "killed after a spill (or an overflow or free-list write), before the meta page" to the crash harness's cut points.
- **Full-length steady-state runs** were not made in this initiative (the owner's request).