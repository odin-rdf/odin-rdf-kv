...
- The memory budget (step 6) and crash testing (step 7) remain. Step 6 is planned as KV-I-0004 (2026-09-24), shaped by the first deployment: about 250 processes per server, each embedding a store, with low, request-driven load. Its one departure from this document: **there is no sweeper thread**; the application calls `env_sweep` (for example after each request, and with a target of 0 to put an idle store to sleep), and a read past the hard watermark evicts inline. This section's design is amended when the initiative completes. *(2026-09-24: done; see the next item.)* *(2026-09-25: step 7 done too; see the item after it.)*
...
- KV-I-0005 (build step 7, crash tests and fuzzing) is implemented (2026-09-25, awaiting the owner's review; *reviewed, completed and archived the same day*). **All seven build steps are done.** A test-only I/O hook (`-define:KV_IO_HOOK=true`) records every write, sync and truncate the store makes; crash images are built from that journal after the fact and opened with the real `env_open`. `-define:KV_NO_SYNC=true` skips the system call in `os_sync` for tests and CI. The suite has 164 tests, 183 in the hooked build, on macOS arm64 and Linux arm64 and amd64. Its Results section has the evidence and the measurements:
  - **Crash safety, process kill:** every cut point of five workloads (a plain commit; a commit that spills, writes overflow runs and a multi-page free-list run and grows the file; deletes that merge and collapse the root; three commits reusing pages; creating a file) reopens at exactly the last committed state, or the next one once its meta page is written, and then passes `tree_check`, `space_check`, one more commit and a reopen: 182 cuts by default. Plus a real `SIGKILL` of a helper process with real syncs, 20 times on one file, on demand.
  - **Crash safety, power loss** (beyond the criterion's words, which say "kill"): with writes after the last sync lost, torn at 512-byte sectors or to a byte prefix, or kept in any subset, every image of workloads 1–4 opens at the last synced commit or, only if its meta page is whole in the image, the next one: 222 images by default, 2,430 with 200 subsets per window. A damaged newest meta page falls back to the previous commit (the S − 1 horizon, KV-I-0002 D1).
  - **Correctness:** the existing oracle model (puts, deletes, aborts, reopens, held readers, both scan directions) now runs as a seeded fuzz mode on demand (`-define:KV_FUZZ=true`): 10⁶ operations in 3–16 s with `KV_NO_SYNC`, a failure reproducing from its seed alone.
  - **Two changes to the store:** a file of exactly two zero pages (a creation cut before its meta pages) is initialised as new instead of `.Corrupted`; and a failed first sync, meta-page write or final sync **poisons** the env: further write transactions return `.Poisoned` until it is reopened (LMDB's `MDB_PANIC`), because the next writer could otherwise overwrite pages a durable meta page points at. The poison test reproduces that corruption without the flag.
  - Known limit: a power loss *during creation* can leave a file that no rule opens (the truncate lost under a kept meta write, or a meta write torn with no whole one beside it). Nothing was ever committed to it; the application fails to start until it is deleted. An open decision for the owner (KV-T-0035). *(Amended 2026-09-25: KV-T-0035 took option 1, a sync after the truncate at creation, so the first class is gone. Only a meta write torn with no whole one beside it remains, and whether that can happen is the owner's open question whether a sector write can tear, KV-T-0038. The kill sweep of creation has one more cut per page size: 184 cuts by default.)*
...
7. Crash tests (kill the process during commit) and fuzzing against a `map[string]string` oracle.
   ...
   *(Done 2026-09-25, KV-I-0005: the kill is simulated at every I/O operation from a recorded journal rather than by killing a process, and a power loss (lost, torn and reordered unsynced writes) is simulated as well; one real `SIGKILL` test runs on demand. The spill workload covers the amendment above, including a cut right after the file grew. Fuzzing is long seeded runs of the existing model rather than a `map[string]string` oracle: the model stores a version and size per key, from which every value is regenerated.)*

## Success Criteria

- **Crash safety:** after a kill during any phase of commit, the database always reopens at the last committed state.
  *(Met 2026-09-25, KV-I-0005: every cut point of five workloads, by kill and by power-loss images; "the last committed state" is exact for a kill, where a commit whose meta page reached the file counts as committed. See its Results. Creation under power loss is the open exception, KV-T-0035.)* *(Amended 2026-09-25: narrowed by KV-T-0035's sync after the truncate to a meta write torn with neither whole, KV-T-0038.)*
- **Correctness:** fuzzed operation sequences match the oracle, including range scans in both directions.
  *(Met 2026-09-25, KV-I-0005: the model test compares every key and both scan directions after every transaction, and the fuzz mode runs it by seed for 10⁶ operations and more. A deliberate off-by-one in `overflow_write` that the ordinary model tests missed at three of six seeds was caught by the fuzz mode at each of those three, within 10⁶ operations.)*
...
- Runs on both macOS and Linux.
  *(2026-09-25: every test passes on macOS arm64 and Linux arm64 and amd64; a GitHub workflow on ubuntu and macOS runners is planned, KV-I-0005 D9.)*
