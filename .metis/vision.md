...
- KV-I-0005 (build step 7, crash tests and fuzzing) is implemented (2026-09-25, awaiting the owner's review). **All seven build steps are done.** A test-only I/O hook (`-define:KV_IO_HOOK=true`) records every write, sync and truncate the store makes; crash images are built from that journal after the fact and opened with the real `env_open`. `-define:KV_NO_SYNC=true` skips the system call in `os_sync` for tests and CI. The suite has 164 tests, 183 in the hooked build, on macOS arm64 and Linux arm64 and amd64. Its Results section has the evidence and the measurements:
  ...
  - Known limit: a power loss *during creation* can leave a file that no rule opens (the truncate lost under a kept meta write, or a meta write torn with no whole one beside it). Nothing was ever committed to it; the application fails to start until it is deleted. An open decision for the owner (KV-T-0035).
...
- **Crash safety:** after a kill during any phase of commit, the database always reopens at the last committed state.
  *(Met 2026-09-25, KV-I-0005: every cut point of five workloads, by kill and by power-loss images; "the last committed state" is exact for a kill, where a commit whose meta page reached the file counts as committed. See its Results. Creation under power loss is the open exception, KV-T-0035.)*
