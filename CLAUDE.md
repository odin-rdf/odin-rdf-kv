...
- `.metis/archived/initiatives/KV-I-0005/initiative.md`: step 7, crash tests and fuzzing (completed and archived; tasks KV-T-0026 to KV-T-0033). Decisions D1–D9 (a recording I/O journal behind `KV_IO_HOOK`, kill and power-loss images built from it, `KV_NO_SYNC` for fuzzing and CI, seeds-only fuzzing, recreating an all-zero new file, `.Poisoned` after a failed sync, the CI position). Its **Results** section has the exit-criteria evidence, the sweep measurements, the CI invocation ready to paste, the deviations and the known limitations.
- Backlog filed by KV-I-0005: `KV-T-0035` (bug, **an open owner decision**: a power loss during creation can leave a file that never opens), `KV-T-0034`, `KV-T-0036`, `KV-T-0037` (tech debt, test coverage). *(Amended 2026-09-25: KV-T-0035 is done, option 1: `init_meta_pages` syncs after the truncate. What remains, a meta write torn with neither whole, is **`KV-T-0038`**, the owner's open question whether a sector write can tear.)*

**Next up:** the owner's call. All seven build steps are done and KV-I-0005 is archived. Open: the sector-atomicity question (KV-T-0038) and the test-coverage backlog (KV-T-0034, KV-T-0036, KV-T-0037).
...
```sh
scripts/test.sh                 # debug, -o:speed, -sanitize:address, debug with KV_IO_HOOK + KV_NO_SYNC, plus odin check for darwin/linux × arm64/amd64
...
scripts/test-linux.sh arm64     # Linux container, debug and -o:speed, each also with KV_IO_HOOK + KV_NO_SYNC; also amd64 (emulated)
```
...
- **The crash sweeps (KV-T-0027 to KV-T-0029):** registered only with `-define:KV_IO_HOOK=true`, so the hooked run of each script is where they run; they need no other define. The default sweep (kill and power-loss images over workloads 1–5, plus the meta-corruption images) takes about 0.27 s at `-o:speed` and 0.9 s debug, one thread, on macOS arm64. `-define:KV_CRASH=true` selects larger trees (about 10.5 s at `-o:speed` on macOS, 16 s on Linux arm64, most of it the kill sweep's reuse workload); `-define:KV_CRASH_SUBSETS=n` (default 16) sets the random power-loss images per window. A failure names the workload, the cut, the image and, for a random image, the seed, the subset and the tears; reproduce it with `-define:ODIN_TEST_RANDOM_SEED=`:
  `odin test tests -o:speed -define:KV_IO_HOOK=true -define:KV_NO_SYNC=true -define:KV_CRASH=true -define:ODIN_TEST_THREADS=2`.
- **CI (KV-I-0005 D9), `.github/workflows/ci.yml`, `ubuntu-latest` only** (the owner, 2026-09-25: a macOS runner costs ten times as much; macOS is covered by `scripts/test.sh` before every push): one build, `odin test tests -o:speed -define:KV_IO_HOOK=true -define:KV_NO_SYNC=true`, plus the cross-target `odin check`s of `scripts/test.sh`; the invocation is written out in KV-I-0005's Results. `scripts/test-linux.sh` runs exactly that configuration too. Nothing on demand (steady, fuzz, kill, criterion, benchmarks, `KV_CRASH`, TSan, ASan) runs in CI.
...
| File | Contents |
|---|---|
| `types.odin` | `Pgno`, `Txn_Id`, `Error` (including `.Poisoned`, KV-I-0005 D7), format constants |
...
| `env.odin` | Open and close, choosing the meta page, recreating an all-zero new file (KV-I-0005 D6), `meta_write`, the reader table, `env_stats`, `file_grow` |
...
- **All file I/O goes through `os_pwrite`, `os_sync` and `os_truncate`** (KV-I-0005 D1): they are what the hook sees, so a write, sync or truncate made any other way is invisible to the journal and to every crash sweep. Opening, mapping and reading are outside the rule; `posix.pwrite`, `ftruncate`, `fsync`, `fdatasync` and `fcntl` appear in `kv/` only in `os_*.odin` (checked by KV-T-0026's grep). A workload in the sweep must assert that it did what it is named for (spilled, merged, reused pages), so a change that stops it doing so fails rather than sweeping less.
- **A new file** (KV-I-0005 D6): a file of size 0, or of exactly two pages at the page size of this open with every byte zero, is initialised as new; anything else without a valid meta page is `.Corrupted` and is left untouched. A power loss during creation can still leave a file that neither rule opens (KV-T-0035, an open decision). *(Amended 2026-09-25: `init_meta_pages` syncs after its truncate (KV-T-0035), so the sizing is durable before a meta page is written; the one creation image left refused is a meta write torn with neither whole, which depends on whether a sector write can tear (KV-T-0038, an open owner decision).)*
...
- `where` and `distinct` are keywords, so they can't be used as parameter names.
