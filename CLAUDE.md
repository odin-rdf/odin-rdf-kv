...
- `.metis/initiatives/KV-I-0005/initiative.md`: step 7, crash tests and fuzzing (tasks KV-T-0026 to KV-T-0033, all done 2026-09-25; the initiative awaits the owner's review and is not yet archived). Decisions D1–D9 (a recording I/O journal behind `KV_IO_HOOK`, kill and power-loss images built from it, `KV_NO_SYNC` for fuzzing and CI, seeds-only fuzzing, recreating an all-zero new file, `.Poisoned` after a failed sync, the CI position). Its **Results** section has the exit-criteria evidence, the sweep measurements, the CI invocation ready to paste, the deviations and the known limitations.
...
**Next up:** the owner's call. All seven build steps are done. Open: the review of KV-I-0005, the decision in KV-T-0035 *(taken 2026-09-25, option 1; the sector-atomicity question is now KV-T-0038)*, and the GitHub workflow (D9; the invocation is in KV-I-0005's Results).
